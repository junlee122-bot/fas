...
## 2026-08-01 후속 보완: 시나리오 조합 용량 77,760개

장기 시뮬레이션의 `budgetPriority`는 실제 국가 운영에서 독립적으로 선택할 수 있는 6개 부문임에도 경제 모델에 종속돼 있었다. 이를 여덟 번째 혼합기수 축으로 승격해 국가별 조합 용량을 12,960개에서 77,760개로 확장했다. 추가 용량은 64,800개로 요청한 20,000개 이상을 충족한다.

- 재건 예산: 번영·지속가능성·정통성 회복
- 복지 예산: 권리·정통성·불안 완화
- 교육 예산: 기술·권리·제도적 회복
- 산업 예산: 번영·기술 증가와 불안·생태 비용
- 외교 예산: 다극성·번영·외부 회복 능력
- 안보 예산: 안보 증가와 권리·정통성·과잉확장 비용

경제 모델 5종과 예산 6종의 30개 교차조합을 보장하고, 77,760개 전체 순환에서 조합 코드와 조합 키가 한 번도 반복되지 않는지 단위 테스트로 고정했다. 국가별 2,000회 재실행에서는 13개국 모두 예산 6종이 표본에 포함됐고, 전체 미래 지문 충돌 0%, 결말 충돌 0.4%, 생존 89.7%, 국가 평균 결과 격차 8.4점으로 모든 회귀 기준을 통과했다. 세부 산식과 다음 확장 후보는 `docs/scenario-combination-capacity-77760.md`에 기록한다.
