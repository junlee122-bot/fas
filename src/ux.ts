...
export function deriveUXActions({
  ...
  if (idleFactories > 0) {
    actions.push({
      id: 'idle-factories',
      priority: 'info',
      title: `민수 공급 여력 ${idleFactories}개`,
      detail: '군수에 추가 배치하면 장비 생산은 늘지만 민간 소비재 공급 여력은 줄어듭니다. 전부 배치할 의무는 없습니다.',
      reason: `가용 ${factories}개 중 ${idleFactories}개는 군수 라인 대신 민수 공급 여력으로 유지됩니다.`,
      ifIgnored: '현재 군수·민수 균형을 유지합니다. 장비 부족과 생활재 공급을 함께 비교하십시오.',
      resolution: '공장 배정 즉시 · 다음 주 장비 생산량에 반영',
      instruction: '본부 현장 보기에서 전환 전 소비재 공급과 장비 생산력을 비교한 뒤 필요한 만큼만 군수에 배치하십시오.',
      label: '생산 조정',
      tab: 'industry',
