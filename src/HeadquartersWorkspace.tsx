...
import { ArrowRight, Building2, ClipboardList, Globe2, Map, Users, X } from 'lucide-react';
...
export function HeadquartersWorkspace(props: HeadquartersWorkspaceProps) {
  ...
  const [view, setView] = useState<'floor' | 'staff' | 'national'>('floor');
  ...
  const staffProps: HeadquartersStaffProps = { ...props, selectedStaffIdentity: identity, onSelectStaff };
  ...
  const viewLabels = { floor: '지휘부 배치도', staff: '참모 업무표', national: '국가 현황' };
  return <section className="hq-workspace" aria-labelledby={`${id}-title`}>
    ...
    <nav className="hq-view-switch" aria-label="현장 작업대">
      {(['floor', 'staff', 'national'] as const).map((value) => <button key={value} type="button" aria-pressed={view === value} onClick={() => setView(value)}>{value === 'floor' ? <Building2 size={17} /> : value === 'staff' ? <Users size={17} /> : <Globe2 size={17} />}{viewLabels[value]}</button>)}
      <span>{input?.role.title ?? '현재 보직 확인 필요'}</span>
    </nav>
    {view === 'national' ? <LivingWorldScene {...world} /> : <>
      <div className={`hq-layout hq-layout--${view}`}>
          ...
          {view === 'staff' ? <HeadquartersStaffTable {...staffProps} /> : <>
            ...
            <div className="hq-floorplan" aria-label="지휘부 시설과 담당자 배치도">
              {headquartersRooms.map((location, index) => {
                ...
                return <div key={location.id} className={`hq-room hq-room--${location.id}${selected || personSelected ? ' is-selected' : ''}`} data-room={location.id}>
                  {index === 4 && <span className="hq-corridor-label" aria-hidden="true">통신·연결 복도</span>}
                  <div className="hq-room-sign"><small>{String(index + 1).padStart(2, '0')} / {access.label}</small><strong>{title(location)}</strong></div>
                  ...
                  <div className="hq-room-floor-status">{getHeadquartersRoomSignal(location, world.input, world.staffIssues, context.formatMoney)}</div>
        ...
        <aside className="hq-detail" id={`${id}-detail`} tabIndex={-1} ref={detailRef} aria-label="선택 상세">
          {identity ? <><button className="hq-back-to-room" type="button" onClick={() => setIdentity(null)}><X size={14} />시설 정보로</button><HeadquartersStaffInspector key={`${identity.id}:${identity.personId}`} {...staffProps} /></> : <section className="hq-facility-inspector">
            ...
            <div className="hq-facility-signal">{getHeadquartersRoomSignal(room, world.input, world.staffIssues, context.formatMoney)}</div>
