...
export function JusticeDocketView(p: JusticeDocketViewProps) {
  ...
  return <section className="justice-command-desk command-edition" data-justice-workspace={selection.workspace} data-compact={Boolean(p.compact)} aria-label="사법 사건 업무공간">
    <header className="jd-heading"><div><small><GameIcon name="justice" size={18} tone="gold" className="inline" />JUSTICE DOCKET · {context.year}</small><h1>사법 사건실</h1><p>{context.role.title} · 제{context.week + 1}주</p></div><span className="jd-badge">미결 {state.cases.filter((item) => item.stage !== 'closed').length}/6 · 결재 {pending ? '대기' : '없음'}</span>{!p.compact ? <div className="governance-illustration-slot"><GameIllustration scene="justice-chamber" compact /></div> : null}</header>
