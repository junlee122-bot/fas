...
import { RegionalLogisticsScene, type RegionalLogisticsView } from './RegionalLogisticsScene';
...
export interface RegionalIndustryBoardProps {
  ...
  initialRequest?: { nationId: RegionalIndustryContext['nationId']; view: RegionalLogisticsView; shipmentId?: string };
}
...
export function RegionalIndustryBoard({ state, context, nationalStockpile, onConfigure, onPlanShipment, onCancelReserved, initialRequest }: RegionalIndustryBoardProps) {
  const account = getRegionalIndustryAccount(state, context.nationId, context.week);
  const presentation = buildRegionalLogisticsPresentation(state, context);
  const visibleIds = new Set(presentation.rows.map((row) => row.id));
  const visibleShipments = account.shipments.filter((shipment) => visibleIds.has(shipment.id));
  const request = initialRequest?.nationId === context.nationId ? initialRequest : undefined;
  ...
  const [view, setView] = useState<RegionalLogisticsView>(request?.view ?? 'overview');
  const [recordFilter, setRecordFilter] = useState<'all' | 'reserved' | 'in-transit' | 'delivered' | 'cancelled'>('all');
  const [selectedRecord, setSelectedRecord] = useState<{ nationId: string; id: string } | null>(request?.shipmentId ? { nationId: context.nationId, id: request.shipmentId } : null);
  const [rejection, setRejection] = useState<{ basis: string; message: string } | null>(null);
  const panelId = useId();
  const workspaceRef = useRef<HTMLElement>(null);
  const focusRequest = useRef(Boolean(request));
  useEffect(() => {
    if (focusRequest.current) {
      workspaceRef.current?.querySelector<HTMLElement>(`[data-regional-view="${view}"]`)?.focus();
  ...
  const activeShipments = visibleShipments.filter((shipment) => shipment.status === 'reserved' || shipment.status === 'in-transit');
  ...
  const records = visibleShipments.filter((shipment) => recordFilter === 'all' || shipment.status === recordFilter);
  const selectedShipment = selectedRecord ? (selectedRecord.nationId === context.nationId ? records.find((shipment) => shipment.id === selectedRecord.id) ?? null : null) : records[0] ?? null;
  const selectedProgress = presentation.rows.find((row) => row.id === selectedShipment?.id);
  const shipmentLabels = getRegionalShipmentRecordLabels(visibleShipments.map((shipment) => ({ ...shipment, heldReason: presentation.rows.find((row) => row.id === shipment.id)?.holdReason ?? null })), context.territories);
  ...
  return <section className="regional-industry" aria-label="지역 집하·수송 시범" ref={workspaceRef}>
    ...
    <div id={`${panelId}-overview`} className="regional-overview" hidden={view !== 'overview'} tabIndex={-1} data-regional-view="overview">
      ...
      {visibleShipments.length > 0 && !quarantined && <RegionalLogisticsScene state={state} context={context} showTotals={false} onOpen={(nextView, shipmentId) => { focusRequest.current = true; setRecordFilter('all'); setSelectedRecord(shipmentId ? { nationId: context.nationId, id: shipmentId } : null); setView(nextView); }} />}
    ...
    <div id={`${panelId}-configure`} className="regional-configuration-view" hidden={view !== 'configure'} tabIndex={-1} data-regional-view="configure">
    ...
    {approved.mode === 'pilot' ? <>
      <div id={`${panelId}-dispatch`} className="regional-dispatch-view" hidden={view !== 'dispatch'} tabIndex={-1} data-regional-view="dispatch">
      ...
      <div id={`${panelId}-records`} className="regional-records-view" hidden={view !== 'records'} tabIndex={-1} data-regional-view="records">
      <div className="regional-record-selectors"><label>운송 상태 필터<select value={recordFilter} onChange={(event) => setRecordFilter(event.target.value as typeof recordFilter)}><option value="all">전체 기록</option>{Object.entries(recordLabels).map(([id, label]) => <option value={id} key={id}>{label}</option>)}</select></label><label>운송 기록 선택<select value={selectedShipment?.id ?? ''} disabled={!records.length} onChange={(event) => setSelectedRecord({ nationId: context.nationId, id: event.target.value })}>{!selectedShipment && <option value="">{records.length ? '운송을 다시 선택하십시오' : '해당 상태의 기록 없음'}</option>}{records.map((shipment) => <option key={shipment.id} value={shipment.id}>{shipmentLabels.get(shipment.id)}</option>)}</select></label></div>
      {selectedShipment ? <article className="regional-selected-shipment" aria-label={`${selectedShipmentLabel} 운송 상세`}>
        <header><strong>{regionalEquipmentLabels[selectedShipment.equipmentKey]} {selectedShipment.quantity.toLocaleString()}개</strong><span>{selectedProgress?.holdReason ? `${recordLabels[selectedShipment.status]} · 보류` : recordLabels[selectedShipment.status]}</span></header>
        <p>{context.territories.find((t) => t.id === selectedShipment.originTerritoryId)?.name ?? selectedShipment.originTerritoryId} → {context.territories.find((t) => t.id === selectedShipment.destinationTerritoryId)?.name ?? selectedShipment.destinationTerritoryId}</p>
        <dl><div><dt>예약 기록</dt><dd>제{selectedShipment.reservedWeek + 1}주</dd></div><div><dt>현재 진행</dt><dd>{selectedShipment.status === 'delivered' ? selectedShipment.deliveredWeek === null ? '도착 주차 기록 없음' : `제${selectedShipment.deliveredWeek + 1}주 확정 도착` : selectedShipment.status === 'cancelled' ? '예약 취소됨 · 도착 실적 아님' : `도착까지 남은 유효 결산 ${selectedProgress?.remainingSteps ?? selectedShipment.remainingWeeks}회`}</dd></div></dl>
        {selectedProgress?.holdReason && <p className="regional-warning">현재 보류 조건: {selectedProgress.holdReason}</p>}
        {selectedProgress?.recordedHoldReason && <p>지난 결산 보류 기록: {selectedProgress.recordedHoldReason}{!selectedProgress.holdReason ? ' 현재 조건은 회복됐지만 재개 여부는 다음 결산에서 확정됩니다.' : ''}</p>}
        <small>현재 설정이 아닌 이 운송에 기록된 출발·도착지입니다.</small>
        {selectedShipment.status === 'reserved' && <button type="button" disabled={!canManage} aria-label={`${selectedShipmentLabel} 예약 취소`} onClick={() => { if (!canManage) return; if (!onCancelReserved(selectedShipment.id)) setRejection({ basis, message: '예약 취소가 승인되지 않았습니다. 이미 출발했는지 현재 기록을 확인하십시오.' }); else setRejection(null); }}>예약 취소</button>}
      </article> : <p className="regional-empty">선택 가능한 운송을 확인하십시오. 이전 선택이 사라지면 다른 운송에 명령하지 않습니다. 필터 선택은 운송을 생성하지 않습니다.</p>}
