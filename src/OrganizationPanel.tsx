...
import { recruitmentChance, recruitmentScore, sortTalentCandidates } from './recruitment';
import type { TalentMarketSort } from './recruitment';
import {
...
export function OrganizationPanel({
  ...
  const [staffView, setStaffView] = useState<'planner' | 'roster' | 'responsibilities'>('planner');
      ...
      : selectedCandidate.knowledge < 30
        ...
        : selectedCandidate.knowledge < 55
          ...
          : selectedCandidateScore < 72
            ? `설득 점수가 ${72 - selectedCandidateScore} 부족합니다. 관심 명단 등록과 비밀 접촉으로 관계를 높이고 경쟁 제안을 낮추십시오.`
            : '영입 조건을 충족했습니다. 정치력 6과 계약금을 확인한 뒤 정식 제안을 보내십시오.';
        ...
        <div className="recruitment-hub">
          ...
          <section className="recruitment-guide" aria-label="인재 영입 절차">
            ...
            <ol>
              ...
              <li><i>4</i><span><strong>정식 제안</strong><small>정보 55% · 6PP · 계약금</small></span></li>
              <li><i>5</i><span><strong>보직 임명</strong><small>설득 72 이상이면 확정 영입</small></span></li>
            </ol>
          ...
          {selectedCandidate && (
            <section className="candidate-report" role="region" aria-label={`${selectedCandidate.name} 조사 보고서`}>
              ...
              <div className="candidate-report-actions">
                ...
                <button title={selectedCandidateManaged ? '현재 직함의 임명권으로 정식 제안을 보냅니다.' : authority.restrictionReason} disabled={!selectedCandidateManaged || selectedCandidateUnavailable || selectedCandidate.knowledge < 55 || selectedCandidateScore < 72 || game.politicalPower < 6 || game.treasury < selectedCandidate.signingCost} onClick={() => onRecruitCandidate(selectedCandidate.id)}>{selectedCandidateManaged ? <Users size={12} /> : <LockKeyhole size={12} />} 정식 제안 · {formatMoney(selectedCandidate.signingCost)}</button>
              ...
              return (
                <article className={`candidate-card ${candidate.status} ${isExpert ? 'historical-expert' : ''} ${manageable ? '' : 'authority-locked'}`} key={candidate.id}>
                  ...
                  <div className="candidate-actions">
                    ...
                    <button disabled={!manageable || unavailable || candidate.knowledge < 55} title={manageable ? `계약금 ${formatMoney(candidate.signingCost)}으로 협상을 시작합니다.` : authority.restrictionReason} onClick={() => onRecruitCandidate(candidate.id)}>{manageable ? <Users size={10} /> : <LockKeyhole size={10} />} 제안</button>
                  </div>
                </article>
              );
            })}
