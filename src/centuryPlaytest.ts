...
function buildOutliers(sessions: LongHorizonSessionResult[]): CenturyOutlier[] {
  return sessions.map((session) => {
    ...
    const researchDroughtYears = round(session.researchIdleWeeks / 52, 1);
    const reasons = [
...
function buildFindings(aggregate: CenturyAggregate): CenturyFinding[] {
  ...
  return [
    {
      priority: 'P0', id: 'post-2020-content-void', title: '2020년 이후 연구와 세계사 콘텐츠가 사실상 종료됩니다',
    ...
    {
      priority: 'P0', id: 'strategic-mode-collapse', title: '전쟁 경력은 초반에 끝나고 장기 캠페인이 단일 국가운영 모드로 수렴합니다',
      evidence: `평균 전쟁 단계는 ${aggregate.averageWarYears.toFixed(1)}년, 국가운영은 ${aggregate.averageNationYears.toFixed(1)}년으로 전쟁 플레이 비중이 ${aggregate.warPhaseShare.toFixed(1)}%뿐입니다. ${earlyEra?.era ?? '초기'} 연간 전투 ${earlyEra?.battlesPerYear.toFixed(1) ?? 0}회가 후기에는 ${lateEra?.battlesPerYear.toFixed(1) ?? 0}회가 됩니다.`,
    ...
    {
      priority: 'P0', id: 'century-agency-dilution', title: '118년 동안 실질 결정 밀도가 낮아져 시간 진행이 플레이를 대체합니다',
    ...
    {
      priority: aggregate.urgentWeekRate >= 25 || (repeat?.averageLongestRunWeeks ?? 0) >= 52 ? 'P0' : 'P1', id: 'alert-repetition', title: '반복 경고가 장기 과제로 전환되지 않아 주의력을 소모합니다',
    ...
    {
      priority: (lateEra?.nationalScoreCeilingWeekRate ?? 0) >= 20 ? 'P0' : 'P1', id: 'late-game-saturation', title: '후기 국가 지표가 상한이나 균형점에 고착됩니다',
    ...
    {
      priority: aggregate.endingCollisionRate >= 60 ? 'P1' : 'P2', id: 'ending-collision', title: '700개의 가능세계가 제한된 결말 조합으로 충돌합니다',
    ...
    {
      priority: profileSpread >= 20 ? 'P1' : 'P2', id: 'profile-polarization', title: '행동 성향에 따라 장기 성과와 마찰이 과도하게 벌어집니다',
    ...
    {
      priority: aggregate.averageElectionActionsPerElection >= 10 ? 'P1' : 'P2', id: 'election-loop-repetition', title: '장기 선거가 동일 캠페인 행동의 반복으로 변합니다',
...
export function aggregateCenturySessions(sessions: LongHorizonSessionResult[], weeksPlayed = CENTURY_PLAYTEST_WEEKS): CenturyPlaytestRun {
  ...
  const aggregate: CenturyAggregate = {
    ...
    medianResearchDroughtYears: round(percentile(sessions.map((session) => session.researchIdleWeeks / 52), .5)),
